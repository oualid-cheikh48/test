...
export const addNotebook = async (name) => {
  try {
    const response = await axios.post('http://localhost:8080/api/notebooks', {
      title: name
    });
...
export const getNotebooks = async () => {
  try {
    const response = await axios.get('http://localhost:8080/api/notebooks');
...
export const deleteNotebook = async (id) => {
  try {
    await axios.delete(`http://localhost:8080/api/notebooks/${id}`);
  } catch (err) {
...
export const addNote = async (notebookId, title, content) => {
  try {
    const response = await axios.post(`http://localhost:8080/api/notebooks/${notebookId}/notes`, {
      title: title,
      content: content
    });
...
export const getNotes = async (notebookId) => {
  try {
    const response = await axios.get(`http://localhost:8080/api/notebooks/${notebookId}/notes`);
